...
let token =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJwYXlsb2FkIjp7InVpZCI6NTcwMzA2fSwiaWF0IjoxNTk3NjY5NDc1fQ.C2BVb8kPMWl6T5lquMQrAWv1JruHMGyvmZ35z6QlIA4';
...
async function pullMember() {
  ...
  while (page <= all_page) {
    ...
    const { data } = await axios.get(
      `https://truthbet.com/api/m/affiliate/members?page=${page}`,
      {
        headers: {
    ...
    for (let downline of downlines.data) {
      ...
      let data2 = await db.member.findOne({
        where: {
      ...
      if (data2) {
        ...
        let created = await db.member.create({
          username: downline.username,
