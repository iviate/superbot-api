...
async function bet(data) {
  ...
  if (
    botObj.bet_side == 11 &&
    data.playList.findIndex((item) => item == 'RB') == -1
  ) {
    return;
  } else if (
    botObj.bet_side == 12 &&
    data.playList.findIndex((item) => item == 'EO') == -1
  ) {
    return;
  } else if (
    botObj.bet_side == 13 &&
    data.playList.findIndex((item) => item == 'SB') == -1
  ) {
    return;
  } else if (
    botObj.bet_side == 14 &&
    data.playList.findIndex((item) => item == 'ZONE') == -1
  ) {
    return;
  } else if (
    botObj.bet_side == 15 &&
  ...
  if (status == 2) {
    ...
    try {
      ...
      idx.forEach((element) => {
        bPayload['details'].push({
          mode: 0,
          idx: element,
          orders: [{ idx: element, amt: betVal }],
          orderLength: 1,
          totAmt: betVal,
        });
      ...
      if (!is_mock) {
        ...
        if (res == null || res.data.status != 200) {
          ...
          res.data.status == 200 &&
          message.txnDetails != undefined &&
          message.txnDetails[0].success == true
        ) {
          if (botObj.open_zero) {
            turnover += zeroVal;
          }
          ...
          if (
            botObj.bet_side == 14 ||
            (botObj.bet_side == 15 && is_opposite == true)
          ) {
            turnover += betVal * 2;
          } else {
            turnover += betVal;
          ...
          // console.log(
          //   `${botObj.userId} rot bot roud ${data.round} bet success`
          // );
          betFailed = true;
          betting = false;
        } else {
...
async function processResultBet(
  betStatus,
  botTransactionId,
  botTransaction,
  gameResult
  ...
  if (!is_mock) {
    ...
    if (score == 0) {
      if (
        (botObj.bet_side == 14 && current.is_opposite == false) ||
        (botObj.bet_side == 15 && current.is_opposite == true)
      ) {
      ...
      if (
        (botObj.bet_side == 14 && current.is_opposite == true) ||
        (botObj.bet_side == 15 && current.is_opposite == false)
      ) {
    ...
    // console.log(
    //   `rot ${botObj.userId}-${user.ufa_account} wallet ${currentWallet}`
    // );
...
async function registerForEventListening() {
  ...
  let cb = (err, result) => {
    ...
    if (result.action == 'result_bet') {
      ...
      betFailed = false;
      if (
        result.table_id == current.table_id &&
