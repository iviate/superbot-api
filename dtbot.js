...
async function inititalInfo() {
  ...
  while (cookie == null) {
    ...
    try {
      ...
      await axios.get(
        `https://bpweb.semgbow777.com/player/singleDraTable.jsp?dm=1&t=${tableId}&title=1&sgt=3&hall=1`,
        {
          headers: {
            Cookie: cookie,
          },
  ...
  if (
    ...
    while (cookie == null) {
      ...
      try {
        ...
        cookieTime = moment();
        await axios.get(
          `https://bpweb.semgbow777.com/player/singleDraTable.jsp?dm=1&t=${tableId}&title=1&sgt=0&hall=1`,
          {
            headers: {
              Cookie: cookie,
            },
...
async function livePlaying(data) {
  ...
  if (
    dataJson.eventType === 'GP_NEW_GAME_START' &&
    previousEventType !== 'GP_NEW_GAME_START'
  ) {
    previousEventType = 'GP_NEW_GAME_START';
    ...
    if (dataJson.gameRound < 6) {
      ...
      if (isPlay && playRound == dataJson.gameRound) {
        ...
        let remainBet = Math.max(
          WAITNG_TIME - Math.round((moment() - previousGameStartAt) / 1000),
          0
        );
        parentPort.postMessage({
          action: 'start',
        ...
        if (remainBet > 8) {
          let sum =
            predictStats.correct + predictStats.wrong + predictStats.tie;
          let win_percent = 0;
          if (sum != 0) {
            win_percent =
              ((predictStats.correct + predictStats.tie) / sum) * 100;
          }
    ...
    predictStats.predict[playCount - 1] = {
      ...lastPlay,
      isResult: true,
      dataJson,
    };
  ...
  if (currentInfo.round > playCount) {
    if (currentInfo.round < 6) {
      ...
      if (isPlay && playRound == currentInfo.round) {
        axios
          .get(`https://truthbet.com/api/baccarat/${workerData.id}/current`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
