            ...
            if (result) {
                ...
                if (WEB == 4) {
                    (async (USERNAME, PASSWORD, WEB) => {
                        ...
                        console.log(`imba77 login ${USERNAME}`)
...
myApp.get('/wallet/:id', async function (request, response) {
    ...
    if (user.is_mock) {
        ...
        //     await page.goto('https://imba77.com/login?token=' + user.token, {
        //         waitUntil: "networkidle2"
        //     });
        ...
        //     let walletAPI = `https://imba77.com/member/get_credit_limit?token=${user.token}`
        //     let config = {
        //         headers: {
...
function createBotWorker(obj, playData, is_mock) {
    let cb = (err, result) => {
        ...
        if (result.action == 'bet_success') {
            ...
            socket.emit(`user`, {id: result.data.current.botObj.userId, data: result})
        ...
        if (result.action == 'info') {
            // console.log('bot info')
            socket.emit(`user`, {id: result.userId, data: { ...result, isPlay: isBet, win_percent: win_percent, currentBetData: currentBetData }})
        }
        ...
        if (result.action == 'process_result') {
            ...
            socket.emit(`user`, {id: result.botObj.userId,
                data: {
                action: "bet_result",
                wallet: result.wallet,
...
function createRotBotWorker(obj, playData, is_mock) {
    let cb = (err, result) => {
        ...
        if (result.action == 'info') {
            // console.log('bot info')
            socket.emit(`user`, {id: result.userId, data: { ...result, isPlay: isPlayRot, currentBetData: rotCurrentBetData}})
        }
        // if (result.action == 'stop') {
        ...
        if (result.action == 'process_result') {
            ...
            db.userTransaction.create(userTransactionData)
            socket.emit(`user`, {id: result.botObj.userId, data: {
                action: "bet_result",
                wallet: result.wallet,
                playData: result.playData,
                status: result.status,
...
function createDtWorker(obj, playData, is_mock) {
    let cb = (err, result) => {
        ...
        if (result.action == 'info') {
            // console.log('bot info')
            socket.emit(`user`, {id: result.userId, data : { ...result, isPlay: dtIsBet, win_percent: win_percent, currentBetData: currentBetData }})
        }
