...
async function inititalInfo() {
  ...
  while (cookie == null) {
    ...
    try {
      ...
      cookieTime = moment();
      await axios.get(
        `https://bpweb.semgbow777.com/player/singleRouTable.jsp?dm=1&t=${tableId}&title=1&sgt=6&hall=1`,
        {
          headers: {
            Cookie: cookie,
          },
  ...
  if (typeof res.data === 'object' && res !== null) {
    ...
    while (cookie == null) {
      ...
      try {
        ...
        cookieTime = moment();
        await axios.get(
          `https://bpweb.semgbow777.com/player/singleRouTable.jsp?dm=1&t=${tableId}&title=1&sgt=0&hall=1`,
          {
            headers: {
              Cookie: cookie,
            },
...
async function livePlaying(data) {
  ...
  if (
    ...
    if (bot != null) {
      ...
      if (
        addition.findIndex((item) => item == bot.TWOZONE[0]) != -1 ||
