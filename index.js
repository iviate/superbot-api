...
io.on('connection', (socket) => {
  ...
  socket.on('restart', (msg) => {
    ...
    if (botWorkerDict.hasOwnProperty(userId) && botWorkerDict != undefined) {
      botWorkerDict[userId].postMessage({
        action: 'restart',
        type: type,
        userId: userId,
      });
    } else if (
      dtBotWorkerDict.hasOwnProperty(userId) &&
      dtBotWorkerDict != undefined
    ) {
      dtBotWorkerDict[userId].postMessage({
        action: 'restart',
        type: type,
        userId: userId,
      });
      ...
      io.emit(`user${userId}`, {
        ...
        data: { success: false, message: 'ยังไม่ได้สร้างบอท', data: null },
      });
    }
  });
...
myApp.get('/wallet/:id', async function (request, response) {
  ...
  if (user.is_mock) {
    ...
    try {
      ...
      //   },
      // };
      let res1 = await getCredit(user.username, user.type_password, user.token);
      // console.log(res.data)
